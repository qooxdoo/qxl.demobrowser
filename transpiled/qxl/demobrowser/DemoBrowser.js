  ...
  qx.Class.define("qxl.demobrowser.DemoBrowser", {
    ...
    */
    construct: function construct() {
      ...
      this.__menuItemStore = {}; // Configure layout
      ...
      this.widgets = {};
      this.tests = {};
      this.__currentTheme = "qx.theme.Indigo"; // Commands & Menu Bar

      this.__makeCommands();

      this.__menuBar = this.__makeToolBar();
      this.add(this.__menuBar); // Main Split Pane
      ...
      stack.exclude(); // Back button and bookmark support
      ...
      this.__infoWindow = new qxl.demobrowser.InfoWindow(this.tr("Info"));
    ...
    */
    members: {
       ...
       *
       */
      __syncRightView: function __syncRightView(e) {
       ...
       *
       */
      __getObjectSummary: function __getObjectSummary() {
      ...
      __openWindow: function __openWindow() {
        var sampUrl = this._iframe.getSource(); // remove th query params


        sampUrl = sampUrl.substr(0, sampUrl.indexOf("?")); // add the current theme as env setting
      ...
      __setCurrentJSCode: function __setCurrentJSCode(code) {
        var playable = !!code;
      ...
      __toPlayground: function __toPlayground() {
      ...
      __makeToolBar: function __makeToolBar() {
        ...
        bar.add(externLinksPart1); // -- spin-out sample
        ...
        externLinksPart1.add(sobutt); // -- to playground

        {
          ...
          playgroundButton.setEnabled(false); // Loading demos into IE fails most of the time because IE truncates
          // long URLs
          ...
          bar.add(externLinksPart2); // api button
          ...
          apiButton.addListener("execute", this.__onApiOpen, this); // help button
        ...
        bar.addListener("showItem", this._onShowItem, this); // DONE
        // -----------------------------------------------------
      ...
      __makeDemoView: function __makeDemoView() {
      ...
      __makeHtmlCodeView: function __makeHtmlCodeView() {
      ...
      __makeJsCodeView: function __makeJsCodeView() {
       ...
       * @return {var} TODOC
       */
      __makeTree: function __makeTree() {
      ...
      __makeUrlMenu: function __makeUrlMenu() {
      ...
      leftReloadTree: function leftReloadTree(e) {
        ...
        var that = this; // Handle current Tree Selection and Content
      ...
      __ehIframeLoaded: function __ehIframeLoaded() {
      ...
      __onLogInterval: function __onLogInterval(e) {
        var fwindow = this._iframe.getWindow();
        ...
        try {
          ...
          if (fwindow && fwindow.qx && fwindow.qx.log && fwindow.qx.log.appender) {
            if (!this.__logDone) {
              ...
              this._history.addToHistory(this._currentSample.replace("/", "~"), document.title); // load sample source code
       ...
       * @return {String} TODOC
       */
      __getPageSource: function __getPageSource(url) {
        ...
        req.addListener("success", function (evt) {
          ...
          var content = evt.getTarget().getResponse(); // if there is a content
          ...
          if (content) {
            {
              ...
              u += "." + cat + "." + base + ".src.js";
              var jsSourceFileName = qx.$$appRoot + u; // get the javascript code

              var reqJSFile = new qx.io.request.Xhr(jsSourceFileName);
              reqJSFile.setTimeout(18000);
              reqJSFile.addListener("success", function () {
                var jsCode = reqJSFile.getResponse(); // store the current visible code
              ...
              }, this); // send the request for the javascript code

              reqJSFile.send(); // write the html code to the html page

              this.widgets["outputviews.sourcepage.html.page"].setHtml(this.__beautySource(content));
            }
          }
        }, this); // add a listener which handles the failure of the request

        req.addListener("fail", function (evt) {
          this.error("Couldn't load file: " + url);
        }, this); // send the request for the html file
      ...
      dataLoader: function dataLoader(url) {
        ...
        req.addListener("success", function (evt) {
          var content = evt.getTarget().getResponse(); // For reasons unknown, requests made here that fail because of security
      ...
      __beautySource: function __beautySource(src, type) {
      ...
      __beautyHtml: function __beautyHtml(str) {
        var res = str; // This match function might be a bit of overkill right now, but provides
        ...
        function matchfunc(vargs) {
          ...
          var curr;
          var endT = false; // handle rest of submatches
      ...
      __onChangeTheme: function __onChangeTheme(e) {
    ...
    */
    destruct: function destruct() {
      ...
      this._disposeObjects("mainsplit", "tree1", "left", "runbutton", "toolbar", "f1", "f2", "_history", "logappender", '_cmdObjectSummary', '_cmdRunSample', '_cmdPrevSample', '_cmdNextSample', '_cmdSampleInOwnWindow', "_navPart", "_runbutton", "__sobutt", "__themePart", "__themeMenu", "__viewPart", "__viewGroup", "__menuBar", "_infosplit", "_searchTextField", "_status", "_tree", "_iframe", "_demoView", "__menuElements", "__logSync", "_leftComposite", "_urlWindow", "_nextButton", "_prevButton", "__menuItemStore", "__overflowMenu");
    }
  });
  qxl.demobrowser.DemoBrowser.$$dbClassInfo = $$dbClassInfo;
})();

//# sourceMappingURL=DemoBrowser.js.map?dt=1586350621714