(function () {
  ...
  ************************************************************************ */
  ...
  qx.Class.define("qxl.demobrowser.demo.data.Finder", {
    ...
    members: {
      main: function main() {
        qxl.demobrowser.demo.data.Finder.prototype.main.base.call(this); // create and configure a window
        ...
        win.setShowClose(false); // open the window

        win.open(); // create and add the lists
        ...
        win.add(list3); // create the controllers, one for each list
        ...
        controller3.setLabelPath("name"); // create the data store

        var url = qx.util.ResourceManager.getInstance().toUri("demobrowser/demo/data/finder.json");
        var store = new qx.data.store.Json(url); // connect the store and the first controller

        store.bind("model.files", controller1, "model"); // connect the rest of the controllers
...
//# sourceMappingURL=Finder.js.map?dt=1586199386327