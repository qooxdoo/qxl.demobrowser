(function () {
  ...
  ************************************************************************ */
   ...
   * @tag serialization
   */
  qx.Class.define("qxl.demobrowser.demo.data.Form", {
    extend: qx.application.Standalone,
    members: {
      main: function main() {
        qxl.demobrowser.demo.data.Form.prototype.main.base.call(this); // create the UI ///////////////////
        // groupbox
        ...
        groupBox.add(okLabel, {
          row: 2,
          column: 0
        });
        var okCheckBox = new qx.ui.form.CheckBox();
        groupBox.add(okCheckBox, {
          row: 2,
          column: 1
        }); // serialize button
        ...
        var model = qx.data.marshal.Json.createModel(modelSkeleton); // create the controller

        var controller = new qx.data.controller.Object(model); // connect the name

        controller.addTarget(nameTextfield, "value", "name", true); // connect the select box

        controller.addTarget(genderSelectBox, "modelSelection[0]", "gender", true //, genderModel2Selection, genderSelection2Model
        ); // connect the checkbox (boolean to int conversion)

        var okModel2CheckBox = {
          converter: function converter(data) {
            return data === 1;
          }
        };
        var okCheckBox2Model = {
          converter: function converter(data) {
            return data ? 1 : 0;
          }
        };
        controller.addTarget(okCheckBox, "value", "ok", true, okModel2CheckBox, okCheckBox2Model); ////////////////////////////////////
        // validation //////////////////////
        // mark the fields as required

        nameTextfield.setRequired(true);
        okCheckBox.setRequired(true); // create the manager

        var manager = new qx.ui.form.validation.Manager();
        nameTextfield.setRequired(true);
        manager.add(nameTextfield);
        manager.add(okCheckBox); // validate the select box
  ...
  qxl.demobrowser.demo.data.Form.$$dbClassInfo = $$dbClassInfo;
})();

//# sourceMappingURL=Form.js.map?dt=1586350623828