(function () {
  ...
  ************************************************************************ */
  ...
  qx.Class.define("qxl.demobrowser.demo.data.ModelDebugging", {
    ...
    members: {
      main: function main() {
        qxl.demobrowser.demo.data.ModelDebugging.prototype.main.base.call(this); // fetch some data from Twitter

        var store = new qxl.demobrowser.demo.data.store.Identica("linuxfoundation"); // create an html embed to view the model
        ...
        store.addListener("loaded", function () {
          var model = store.getModel(); // display the model in the log

          this.debug(qx.dev.Debug.debugProperties(model)); // display the model in an html embed
        ...
        description.setValue("<b>Debugging models</b><br/>Every model created by the data stores consists only of qooxdoo objects with properties. To see the properties, you can use the debugModel function in qx.dev.Debug.");
  ...
  qx.Class.define("qxl.demobrowser.demo.data.store.Identica", {
    ...
    construct: function construct(user) {
      var url = "http://identi.ca/api/statuses/user_timeline/" + user + ".json";
      qx.data.store.Jsonp.constructor.call(this, url);
    }
  });
  qxl.demobrowser.demo.data.ModelDebugging.$$dbClassInfo = $$dbClassInfo;
})();

//# sourceMappingURL=ModelDebugging.js.map?dt=1589490212632