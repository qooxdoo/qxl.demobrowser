  ...
  qx.Class.define("qxl.demobrowser.demo.table.Table_Window_Editor", {
    ...
    members: {
      ...
      createTable: function createTable() {
        ...
        tableModel.setColumnEditable(1, true); // Customize the table column model.  We want one that automatically
        // resizes columns.
        ...
        var tcm = table.getTableColumnModel(); // Obtain the behavior object to manipulate

        var resizeBehavior = tcm.getBehavior(); // This uses the set() method to set all attributes at once; uses flex
        ...
        resizeBehavior.setMaxWidth(1, 320); // Set one fixed width column

        resizeBehavior.set(3, {
          width: 100
        }); // Display a checkbox in column 3

        tcm.setDataCellRenderer(3, new qx.ui.table.cellrenderer.Boolean()); // We don't want the focus indicator.

        table.setShowCellFocusIndicator(false); // Specify the cell editor factory.  Can't use default (or,
  ...
  qx.Class.define("qxl.demobrowser.demo.table.ModalCellEditorFactory", {
    ...
    members: {
      ...
      createCellEditor: function createCellEditor(cellInfo) {
        ...
        cellEditor.add(cellEditor.__cellEditor); // Create the "Save" button to close the cell editor

        var save = new qx.ui.form.Button("Save");
        save.addListener("execute", function (e) {
          cellEditor.close();
        });
        cellEditor.add(save); // Let them press Enter from the cell editor text field to finish.
...
//# sourceMappingURL=Table_Window_Editor.js.map?dt=1586350627884