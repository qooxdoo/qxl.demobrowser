(function () {
  ...
  ************************************************************************ */
  qx.Class.define("qxl.demobrowser.demo.util.FSMMaze", {
    ...
    construct: function construct(rows, columns, x, y, cellSize) {
      ...
      this.cells = []; // Each element of mazeInfo is a bitmap of walls
      // (qxl.demobrowser.demo.util.FSMMaze.Direction.*)

      this.mazeInfo = []; // Build a grid with all walls of all cells intact
      ...
      var currentCell;
      var neighbors;
      var neighbor; // Start with some random cell
      ...
      while (visitedCells < totalCells) {
        ...
        neighbors = []; // See if there's a west neighbor with all walls intact

        if (currentCell.col > 0 && this.mazeInfo[currentCell.row][currentCell.col - 1] == 0xf) {
          neighbors.push({
            row: currentCell.row,
        ...
        if (currentCell.row < this.numRows - 1 && this.mazeInfo[currentCell.row + 1][currentCell.col] == 0xf) {
          neighbors.push({
        ...
        if (currentCell.col < this.numCols - 1 && this.mazeInfo[currentCell.row][currentCell.col + 1] == 0xf) {
          neighbors.push({
            row: currentCell.row,
            col: currentCell.col + 1,
        ...
        if (currentCell.row > 0 && this.mazeInfo[currentCell.row - 1][currentCell.col] == 0xf) {
          neighbors.push({
            row: currentCell.row - 1,
            col: currentCell.col,
        ...
        if (neighbors.length > 0) {
          ...
          neighbor = neighbors[r]; // Knock down the wall between it and currentCell.  This is a
          // multiple-step process:
          // Step 1: Remove the wall flag on the current cell

          this.mazeInfo[currentCell.row][currentCell.col] &= ~neighbor.currentCellWall; // Step 2; Remove the wall flag on the neighbor cell

          this.mazeInfo[neighbor.row][neighbor.col] &= ~neighbor.neighborWall; // Step 3: Actually remove the wall on the current cell
          ...
          this.cells[currentCell.row][currentCell.col].setDecorator(currentWall); // Step 4: Actually remove the wall on the neighbor cell
          ...
          this.cells[neighbor.row][neighbor.col].setDecorator(neighborWall); // Push currentCell onto the cell stack
      ...
      this.startCell = {
        row: Math.floor(Math.random() * this.numRows),
        col: Math.floor(Math.random() * this.numCols) // Show the starting cell

      };
      var startCell = this.cells[this.startCell.row][this.startCell.col];
      startCell.setBackgroundColor("#b0ffb0"); // Determine the ending cell, not too close to the starting cell
      ...
      } while (Math.abs(this.startCell.row - this.endCell.row) < this.numRows / 2 || Math.abs(this.startCell.col - this.endCell.col) < this.numCols / 2); // Show the ending cell
    ...
    members: {
       ...
       */
      getStartCell: function getStartCell() {
       ...
       *   The returned object contains two members: row and col.
       */
      getEndCell: function getEndCell() {
        return this.endCell;
      },
       ...
       */
      getCellTopLeft: function getCellTopLeft(cell) {
        return {
       ...
       */
      getWestCell: function getWestCell(cell) {
       ...
       *   If there is no such neighbor, null is returned.
       */
      getSouthCell: function getSouthCell(cell) {
        var dir = qxl.demobrowser.demo.util.FSMMaze.Direction.SOUTH;

        if (cell.row < this.numRows - 1 && (this.mazeInfo[cell.row][cell.col] & dir) == 0) {
          return {
            row: cell.row + 1,
            col: cell.col
          };
       ...
       *   If there is no such neighbor, null is returned.
       */
      getEastCell: function getEastCell(cell) {
        var dir = qxl.demobrowser.demo.util.FSMMaze.Direction.EAST;

        if (cell.col < this.numCols - 1 && (this.mazeInfo[cell.row][cell.col] & dir) == 0) {
          return {
            row: cell.row,
            col: cell.col + 1
          };
       ...
       *   If there is no such neighbor, null is returned.
       */
      getNorthCell: function getNorthCell(cell) {
       ...
       * @return {Void}
       */
      markCell: function markCell(cell) {
...
//# sourceMappingURL=FSMMaze.js.map?dt=1586350629003