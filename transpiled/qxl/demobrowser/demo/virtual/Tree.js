(function () {
  ...
  ************************************************************************ */
   ...
   */
  qx.Class.define("qxl.demobrowser.demo.virtual.Tree", {
    extend: qx.application.Standalone,
    members: {
      i: 0,
      __store: null,
      main: function main() {
        qxl.demobrowser.demo.virtual.Tree.prototype.main.base.call(this); // initializes layout
        ...
        container.add(tree); // tree drag & drop
        ...
        }, this); // loads the tree model

        var url = "json/tree.json";
        var store = new qx.data.store.Json(url);
        this.__store = store; // connect the store and the tree

        store.bind("model", tree, "model"); // opens the 'Desktop' node
      ...
      removeItem: function removeItem(root, item) {
      ...
      getCommandFrame: function getCommandFrame(tree) {
        ...
        commandFrame.add(new qx.ui.core.Spacer(spacerSize, spacerSize), {
          row: row++,
          column: 0
        });
        commandFrame.add(new qx.ui.basic.Label("Open mode:"), {
          row: row,
          column: 0
        });
        var modes = {
        ...
        commandFrame.add(new qx.ui.basic.Label("Selection:"), {
          row: row,
          column: 0
        });
        var btnMultiSelect = new qx.ui.form.CheckBox("Enable multi selection");
        commandFrame.add(btnMultiSelect, {
          row: row++,
          column: 1
        });
        ...
        commandFrame.add(btnDragSelect, {
          row: row++,
          column: 1
        });
        ...
        commandFrame.add(new qx.ui.basic.Label("Root node:"), {
          row: row,
          column: 0
        });
        var btnHideRoot = new qx.ui.form.CheckBox("Hide Root Node");
        commandFrame.add(btnHideRoot, {
          row: row++,
          column: 1
        });
        ...
        commandFrame.add(btnShowTopLevelOpenCloseIcons, {
          row: row++,
          column: 1
        });
        ...
        updateModel.addListener("execute", function (e) {
          ...
          for (var i = 0; i < 10; i++) {
            rawData.push({
              "name": "File #" + (trash.getChildren().getLength() + i)
            });
          }
        ...
        commandFrame.add(updateModel, {
          row: row++,
          column: 1
        });
        var vToggleHeight = new qx.ui.form.Button("Toggle Height");
        commandFrame.add(vToggleHeight, {
          row: row++,
          column: 1
        });
        ...
        commandFrame.add(new qx.ui.basic.Label("Drag me:"), {
          row: row,
          column: 0
        });
        commandFrame.add(ddImg, {
          row: row++,
          column: 1
        });
        var grow = true;
  ...
  qxl.demobrowser.demo.virtual.Tree.$$dbClassInfo = $$dbClassInfo;
})();

//# sourceMappingURL=Tree.js.map?dt=1589490217642