...
compiler.command.addListener("made",  (e) => new qx.Promise((fullfiled) => {
    ...
    const config = {
      ...
      verbose: compiler.command.argv.verbose
    };
    let appInfos = [];
    ...
    async.series([
      ...
      (cb) => {
         ...
         let clazz = compiler.command._getMaker().getTarget().constructor;
         let target = new clazz(output);
         if (compiler.inputData.target.type === "build") {
...
compiler.configuration = compiler.inputData;