...
qx.Class.define("qxl.demobrowser.demo.util.PropertyGroup", {
  ...
  construct(propertyDescription) {
    ...
    for (var prop in this._properties) {
      ...
      this._add(
        new qx.ui.basic.Label(prop).set({
          paddingTop: 4,
        }),
        { row: row, column: 0 }
      );
      ...
      if (nullable) {
        ...
        nullWidget.addListener(
          "changeValue",
          this._createOnNullPropertyChange(prop),
          this
        );
      ...
      if (type == "int") {
        var formItem = new qx.ui.form.Spinner().set({
          ...
          maximum:
            this._properties[prop].max !== undefined
              ? this._properties[prop].max
              : 1000,
        });

        formItem.addListener(
          "changeValue",
          this._createOnIntPropertyChange(prop),
          this
        );
        ...
        formItem.addListener(
          "changeValue",
          this._createOnBoolPropertyChange(prop),
          this
        );
        this._add(formItem, { row: row++, column: 1 });
      } else if (type == "enum") {
        ...
        formItem.addListener(
          "changeSelection",
          this._createOnEnumPropertyChange(prop, formItem),
          this
        );
      } else if (type == "string") {
  ...
  members: {
    ...
    _createOnIntPropertyChange(property) {
      return function (e) {
    ...
    _createOnBoolPropertyChange(property) {
      return function (e) {
        var widget = this.getSelected();
        this._setProperty(widget, property, e.getTarget().getValue());
      };
    ...
    _createOnEnumPropertyChange(property, mgr) {
      return function (e) {
        var widget = this.getSelected();
        this._setProperty(
          widget,
          property,
          mgr.getSelection()[0].getUserData("value")
        );
      };
    },

    _createOnNullPropertyChange(property) {
      return function (e) {
        var widget = this.getSelected();
        ...
        if (control.getValue()) {
          ...
          if (this._properties[property].type == "enum") {
            value = this._properties[property].formItem
              .getSelection()[0]
              .getUserData("value");
          } else {
