  ...
  members: {
    main() {
      ...
      if (qx.core.Environment.get("html.video.h264")) {
        var videoUrl = qx.util.ResourceManager.getInstance().toUri(
          "demobrowser/demo/media/qx.m4v"
        );
      } else if (qx.core.Environment.get("html.video.webm")) {
        var videoUrl = qx.util.ResourceManager.getInstance().toUri(
          "demobrowser/demo/media/qx.webm"
        );
      } else if (qx.core.Environment.get("html.video.ogg")) {
        var videoUrl = qx.util.ResourceManager.getInstance().toUri(
          "demobrowser/demo/media/qx.ogv"
        );
      } else {
        doc.add(
          new qx.ui.basic.Label(
            "It seems that your browser doesn't support HTML5 video",
            { left: 10, top: 10 }
          )
        );
        return;
      ...
      video.addListener(
        ...
        function () {
          ...
          container
            .getContentElement()
            .getDomElement()
            .appendChild(video.getMediaObject());
